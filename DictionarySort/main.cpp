...
namespace ParallelMergeSort {
    ...
    void partition(bool parity, ArrayT & array, ArrayT & temporary, ComparatorT & comparator, std::size_t lowerBound, std::size_t upperBound, std::size_t threaded);
    ...
    struct ParallelPartition {
        ...
        void operator()() {
            partition(parity, array, temporary, comparator, lowerBound, upperBound, threaded);
        }
    };
    ...
    template <typename ArrayT, typename ComparatorT>
    void partition(bool parity, ArrayT & array, ArrayT & temporary, ComparatorT & comparator, std::size_t lowerBound, std::size_t upperBound) {
        ...
        if (count == 1) {
            if (parity) {
                temporary[lowerBound] = array[lowerBound];
            } else {
                array[lowerBound] = temporary[lowerBound];
            }
        } else if (count > 1) {
            std::size_t middle = (lowerBound + upperBound) / 2;
            
            partition(!parity, array, temporary, comparator, lowerBound, middle);
            partition(!parity, array, temporary, comparator, middle, upperBound);
            
            if (parity) {
                merge(array, temporary, comparator, lowerBound, middle, upperBound);
            } else {
                merge(temporary, array, comparator, lowerBound, middle, upperBound);
            }
        }
        
        // If we get here, we are at the bottom of the tree.
        // By default, data is in both array and temporary.
    }
    ...
    void partition(bool parity, ArrayT & array, ArrayT & temporary, ComparatorT & comparator, std::size_t lowerBound, std::size_t upperBound, std::size_t threaded) {
        ...
        if (lowerBound < (upperBound - 1)) {
            ...
            if (threaded > 0) {
                ...
                if (USE_THREADS) {
                    ...
                    ParallelPartition<ArrayT, ComparatorT> 
                        lowerPartition = {!parity, array, temporary, comparator, lowerBound, middle, threaded - 1}, 
                        upperPartition = {!parity, array, temporary, comparator, middle, upperBound, threaded - 1};
                    ...
                    partition(!parity, array, temporary, comparator, lowerBound, middle, threaded - 1);
                    partition(!parity, array, temporary, comparator, middle, upperBound, threaded - 1);
                }
                
                //std::cerr << "Partition time: " << tp.time() << std::endl;
            } else {
                partition(!parity, array, temporary, comparator, lowerBound, middle);
                partition(!parity, array, temporary, comparator, middle, upperBound);
            }
            ...
            //std::cerr << "Merge time: " << tm.time() << std::endl;
        }
    }
    ...
    void sort(ArrayT & array, ComparatorT & comparator, std::size_t threaded = 2) {
        ...
        //temporary.resize(array.size());
        
        if (threaded == 0)
            partition(false, array, temporary, comparator, 0, array.size());
        else
            partition(false, array, temporary, comparator, 0, array.size(), threaded);
    }
}
    ...
    public:
        ...
        void sort(const WordsT & input, WordsT & output)
        {
            ...
            for (typename WordsT::const_iterator i = input.begin(); i != input.end(); ++i) {
                ...
                // generation of word order is distributed across multiple processors.
            ...
            ParallelMergeSort::sort(words, comparator, 1);
...
{    
    typedef DictionarySort::Dictionary<char> ASCIIDictionaryT;
    std::string s = "abcdefghijklmnopqrstuvwxyz";
    ASCIIDictionaryT::WordT alphabet(s.begin(), s.end()), w1, w2, w3;
